...
void ApplVPlatoonMg::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        // register executeEachTimeStepSignal and subscribe
        executeEachTS = registerSignal("executeEachTimeStepSignal");
...
void ApplVPlatoonMg::receiveSignal(omnetpp::cComponent *source, omnetpp::simsignal_t signalID, long i, cObject* details)
{
    ...
    if(intersectionManagement)
    {
        ...
        if(vehicleState == state_platoonLeader)
        {
            ...
            if(!haveSendPltInfo)
            {
                ...
                LOG_INFO << boost::format("%s enter intersection area, send PltInfo at currentTime: %.2f\n")%SUMOID%currentTime << std::flush;
                // call sendPltInfo
...
//            std::cout << "plnSize:" << plnSize << std::endl;
            ...
            } else if (wsm->getOptSize() > plnSize) {
                intersectionMerge = true;
            }
            optPlnSize = wsm->getOptSize();
            LOG_INFO << boost::format("%s receive PltCtrl at time: %.2f: optPlnSize: %d, refSpeed: %.2f, refAcc:%.2f\n")
                        %SUMOID%omnetpp::simTime().dbl()%optPlnSize%wsm->getRefSpeed()%wsm->getRefAcc()<< std::flush;

            // for the leader of a platoon, change speedmode to ignore stop line
            ...
            if(leader.leaderID != "")
            {
                if( leader.distance2Leader > 100 )
                {
                    ...
                    std::cout << SUMOID << " change speed mode to 6" << std::endl;
                }
            }
            else
            {
            ...
            // WAIT stage: set max decel to refAcc
            if(wsm->getRefAcc() < 0) {
            ...
            // check each updateInterval to check
...
{
    // accel & decel set for intersectionMerge situation
    // for later platoons (gapcheck part)
    // for leader in first platoon (refspeed part)
    bool gapCheck = false;
    if(!accelRestore) {
        auto leader = TraCI->vehicleGetLeader(SUMOID, sonarDist);
        ...
        {
            TraCI->vehicleSetMaxDecel(SUMOID, MAX_DECEL);
            TraCI->vehicleSetMaxAccel(SUMOID, MAX_ACCEL);
            std::cout << SUMOID << " change MaxAccel to "<< MAX_ACCEL << ", Decel to " << MAX_DECEL << std::endl;
            accelRestore = true;
        }
    }
    ...
    if(!optSizeRestore) {
        TraCICoord pos = TraCI->vehicleGetPosition(SUMOID);
        double speed = TraCI->vehicleGetSpeed(SUMOID);
        double totalLen = (plnSize - 1) * TG * speed + plnSize * VEH_LENGTH;
//        double totalLen = 0;
        double curx = pos.x;
        if(abs(EAST_STOP_LINE_X - (pos.x - totalLen)) < 1) {

            std::cout << SUMOID << " arrive maneuver point at " << omnetpp::simTime().dbl() << std::endl;

            TraCI->vehicleSetSpeedMode(SUMOID, 31); // speedmode to default (all checks on)
            std::cout << SUMOID << " change speed mode to 31" << std::endl;

            optPlnSize = originOptSize;
            std::cout << SUMOID << " optSize changed to " << optPlnSize << std::endl;

            if(intersectionSplit) { // only leader (veh) under S1
                TraCI->vehicleSetSpeed(SUMOID, TraCI->vehicleGetSpeed(SUMOID)/2.5);
                std::cout << SUMOID << " speed changed to " << TraCI->vehicleGetSpeed(SUMOID)/2.5 << std::endl;
            }
...
//        scheduleAt(omnetpp::simTime() + updateInterval, intersectionTIMER);
    ...
    if(gapCheck) {
        cancelEvent(intersectionTIMER);
        std::cout << SUMOID << " -> intersectionTIMER cancel" << std::endl;
    } else {
        scheduleAt(omnetpp::simTime() + updateInterval, intersectionTIMER);
    }
}
...
void ApplVPlatoonMg::merge_DataFSM(PlatoonMsg* wsm)
{
    ...
    else if(vehicleState == state_mergeAccepted)
    {
        ...
        if(currentX < 0) {
            // speed up to catch up (intersection merge)
            TraCI->vehicleSetMaxAccel(SUMOID, 0.2);
            TraCI->vehicleSetSpeed(SUMOID, frontVehSpeed * 1.1);  // catch-up
        } else {
            //  speed up to catch up (after-intersection merge)
            TraCI->vehicleSetSpeed(SUMOID, 16.66);  // catch-up
        }
...
void ApplVPlatoonMg::pltSplitMonitor()
{
    ...
    if(vehicleState == state_platoonLeader)
    {
        ...
        {
            LOG_INFO << boost::format("platoon %s is splitting from %d at %d\n") %SUMOID %optPlnSize %omnetpp::simTime().dbl()<< std::flush;
...
void ApplVPlatoonMg::split_DataFSM(PlatoonMsg *wsm)
{
    ...
    else if(vehicleState == state_splitDone)
    {
        ...
        entry.intersectionMerge = intersectionMerge;
        entry.originOptSize = originOptSize;
    ...
    else if(vehicleState == state_waitForSplitDone)
    {
        if(wsm->getUCommandType() == SPLIT_DONE && wsm->getSenderID() == oldPlnID && wsm->getReceiverID() == SUMOID)
        {
            ...
            if(wsm->getValue().manualSplit)
                ...
                if(wsm->getValue().intersectionMerge) {
                    optPlnSize = originOptSize;
                    intersectionMerge = true;
                    std::cout << SUMOID << " optSize changed to " << optPlnSize << " (by FSM)" << std::endl;
                } else {
            ...
            else
            {
                ...
                {
                    // for platoon restore
                    originOptSize = wsm->getValue().originOptSize;
                ...
                {
                    // example
                    // start 8 -> 2 3 3; intersection 8; after 8 -> 2 3 3
                    if(!wsm->getValue().intersectionMerge)
                        originOptSize = wsm->getValue().originOptSize;

                    TraCI->vehicleSetSpeed(SUMOID, 16.66); // set max speed
                    std::cout << "regularSplit/restoreSplit, set speed 16.66" << std::endl << std::endl;
                }
            }
